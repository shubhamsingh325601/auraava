...
export default function EditHairCarePage({ params }: { params: { id: string } }) {
    ...
    const fetchHairCareItem = async () => {
        try {
            ...
            console.error('Failed to fetch hair care item:', error)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                alert('Failed to update hair care tip')
            }
        } catch (error) {
            console.error('Failed to update hair care tip:', error)
            alert('Failed to update hair care tip')
        } finally {
            setSaving(false)
        }
    ...
    return (
        <div className="min-h-screen bg-ivory">
            <AdminPageHeader icon={Sparkles} eyebrow="Hair Care" title="Edit Hair Care Tip" backHref="/admin/hair-care" />
                        ...
                        <div className="space-y-4">
                            {formData.image && (
                                <div className="relative w-full h-64 rounded-xl overflow-hidden border border-border">
                                    <img src={formData.image} alt="Hair Care" className="w-full h-full object-cover" />
                                    <button
                                        type="button"
