...
import { Loader2, Upload, X, Droplet } from "lucide-react"
import { Button } from "@/components/ui/button"
import { AdminPageHeader } from "@/components/admin/page-header"
import { uploadImage } from "@/lib/upload-image"

export default function EditSkincarePage({ params }: { params: { id: string } }) {
    ...
    const fetchSkincareItem = async () => {
        try {
            const res = await adminFetch(`/api/skincare/${params.id}`)
            ...
            if (data.item) {
                setFormData({
                    ...
                    reverse: data.item.reverse,
                    order: data.item.order,
                })
            }
        } catch (error) {
            console.error('Failed to fetch skincare item:', error)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                alert('Failed to update skincare tip')
            }
        } catch (error) {
            console.error('Failed to update skincare tip:', error)
            alert('Failed to update skincare tip')
        } finally {
            setSaving(false)
        }
    ...
    return (
        <div className="min-h-screen bg-ivory">
            <AdminPageHeader icon={Droplet} eyebrow="Skincare" title="Edit Skincare Tip" />
