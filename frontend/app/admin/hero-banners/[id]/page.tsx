...
export default function EditHeroBannerPage({ params }: { params: { id: string } }) {
    ...
    const fetchHeroBanner = async () => {
        try {
            const res = await adminFetch(`/api/hero-banners/${params.id}`)
            const data = await res.json()
            if (data.banner) {
                setFormData({
                    image: data.banner.image,
                    link: data.banner.link || "/products",
                    order: data.banner.order,
                    active: data.banner.active !== false,
                })
            }
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-ivory">
            <AdminPageHeader icon={ImageIcon} eyebrow="Hero Banners" title="Edit Hero Banner" backHref="/admin/hero-banners" />
