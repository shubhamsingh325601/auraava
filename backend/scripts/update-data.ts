// One-shot data sync script for the bugs sheet (run from backend/):
//   MONGODB_URI="<connection-string>" npm run update:data
...
async function main() {
    ...
    // 1. Stats: Happy Customers → 5K, Natural Products → Pan India Delivery 100%, Average Rating → 4.9
    const statsCollection = db.collection('statsdatas')
    const statsDoc = await statsCollection.findOne()
    if (statsDoc) {
        const items = (statsDoc.items as any[]).map((item: any) => {
    ...
    // 2. Founder: designation Founder & CEO → Founder (name left as-is; final name is client-provided)
    ...
    // 3. Products: enable direct checkout (Shop Now / Buy Directly) for Hair Oils + Shampoos
